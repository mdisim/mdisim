...
function QuickActionButton({ action, onClick }: { action: QuickAction; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-2 px-3 py-2 text-left text-[11px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg hover:border-violet-300 dark:hover:border-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/10 transition-all group"
    >
  ...
  return (
    <div className={cn('flex gap-2', isUser ? 'flex-row-reverse' : 'flex-row')}>
      <div className={cn(
        ...
        isUser ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-gradient-to-br from-violet-100 to-blue-100 dark:from-violet-900/30 dark:to-blue-900/30',
      )}>
        {isUser ? <User size={14} className="text-blue-600" /> : <Bot size={14} className="text-violet-600" />}
      </div>
      <div className={cn(
        ...
        {!isUser && (
          <button
            onClick={handleCopy}
            className="absolute top-1 right-1 p-1 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-slate-600 transition-opacity"
            title="Copy"
          >
...
export function CopilotChat({ projectId, projectName, currentPage }: CopilotChatProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-violet-600 to-blue-600 text-white rounded-full shadow-lg hover:shadow-xl hover:from-violet-700 hover:to-blue-700 transition-all group"
        title="AI Copilot"
      >
  ...
  return (
    <div
      className={cn(
        'fixed z-50 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl flex flex-col transition-all duration-200',
        isExpanded
          ? 'inset-4 rounded-2xl'
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-violet-50 to-blue-50 dark:from-violet-900/20 dark:to-blue-900/20 rounded-t-2xl">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">AI Copilot</h3>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-[200px]">{projectName}</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setIsExpanded(!isExpanded)} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors" title={isExpanded ? 'Minimize' : 'Expand'}>
            {isExpanded ? <Minimize2 size={14} /> : <Maximize2 size={14} />}
          </button>
          <button onClick={() => { setIsOpen(false); setIsExpanded(false) }} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
            <ChevronDown size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center pt-4 pb-2">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-violet-100 to-blue-100 dark:from-violet-900/20 dark:to-blue-900/20 flex items-center justify-center mb-3">
              <Sparkles size={26} className="text-violet-500" />
            </div>
            <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1">How can I help?</h4>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 text-center max-w-[280px] mb-4">
              I have full context of this project — drawings, BOQ, measurements, costs, contracts, and more. Ask me anything.
            </p>

            <div className="w-full space-y-1.5">
              <p className="text-[10px] uppercase tracking-wider text-slate-400 font-medium px-1">Quick actions</p>
        ...
        {isLoading && (
          <div className="flex gap-2">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-100 to-blue-100 dark:from-violet-900/30 dark:to-blue-900/30 flex items-center justify-center shrink-0">
              <Bot size={14} className="text-violet-600" />
            </div>
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl rounded-tl-sm px-3 py-2">
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-700">
        <div className="flex items-end gap-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 focus-within:border-violet-300 dark:focus-within:border-violet-600 transition-colors">
          <textarea
            ref={inputRef}
            value={input}
            ...
            rows={1}
            className="flex-1 bg-transparent px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-400 resize-none outline-none max-h-24"
