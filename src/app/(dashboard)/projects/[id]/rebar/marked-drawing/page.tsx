...
export default async function MarkedDrawingPage({
  ...
  const [{ data: project }, elemResult, pagesResult, { data: allDrawings }] = await Promise.all([
    ...
    supabase
      .from('rebar_elements')
      .select('id, element_mark, element_type, source_drawing_id, source_page, bars:rebar_bars(id, bar_mark, diameter_mm, shape_code, quantity, total_weight_kg, ocr_bbox, ocr_confidence, label_x, label_y, notes)')
      .eq('project_id', id)
      .order('sort_order'),
    supabase
